...
function updateNextPrayer() {
  ...
  // Also update countdown display in prayer items
  const nextItems = $$('[data-prayer-key]');
...
function renderHijriToday() {
  ...
  // Use Arabic month names if language is Arabic
  const monthName = state.lang === 'ar' ? 
    HIJRI_MONTHS_AR[h.month - 1] : 
    HIJRI_MONTHS[h.month - 1];
...
async function forwardGeocode(query, limit = 1) {
  try {
    const r = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query)}&limit=${limit}`);
    const d = await r.json();
    if (limit === 1 && d.length === 0) return null;
    if (limit === 1) {
      return {
    ...
    // Return multiple results for suggestions
    return d.map(item => ({
      lat: parseFloat(item.lat),
      lng: parseFloat(item.lon),
      name: item.display_name.split(',')[0].trim(),
      display: item.display_name.split(',').slice(0, 2).join(', ')
    }));
  } catch {
    return limit === 1 ? null : [];
  }
...
function attachEvents() {
  ...
  $('#cityInput').addEventListener('input', async (e) => {
    ...
    suggestionsTimeout = setTimeout(async () => {
      const results = await forwardGeocode(q, 10);
      if (results && results.length > 0) {
        const dropdown = $('#citiesDropdown');
        dropdown.innerHTML = `
          <div class="cities-list">
            ${results.map((city, idx) => `
              <button class="city-btn" data-idx="${idx}" style="text-align: left; display: block; width: 100%;">
                <div style="font-weight: 500;">${city.name}</div>
                <div style="font-size: 11px; opacity: 0.6;">${city.display}</div>
              </button>
            `).join('')}
          </div>
        `;
        ...
        dropdown.querySelectorAll('.city-btn').forEach((btn, idx) => {
          btn.addEventListener('click', async (e) => {
            e.preventDefault();
            const selectedCity = results[idx];
            $('#cityInput').value = selectedCity.name;
            dropdown.classList.add('hidden');
            await setLocation(selectedCity);
            toast(t('locationOk'), 'success');
          });
        });
      }
